...
(function () {
  ...
  function updateGhostIndicators() {
    ...
    if (currentPlayer === 'X' && historyX.length === 3) {
      ...
      if (cells[oldestX]) cells[oldestX].classList.add('ghost-fade');
    } else if (currentPlayer === 'O' && historyO.length === 3) {
      const oldestO = historyO[0];
      if (cells[oldestO]) cells[oldestO].classList.add('ghost-fade');
    }
  }
  ...
  // Modifiers Toggle Handlers (Blitz & Infinite)
